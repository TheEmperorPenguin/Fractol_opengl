...
void main() {
  ...
  vec3 normal = vec3(1.0);
    ...
    for (int i = 0; i < 512; i++) {
        vec3 position = ray_origin + ray_direction * travelled_distance;
        ...
        if (travelled_distance > 100.) break;
    }
    color = vec3(normal);           // color based on distance
